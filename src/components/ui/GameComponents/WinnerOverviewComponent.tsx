...
const WinnerOverviewComponent: React.FC<Props> = (props) => {
        const {
            renderTitle, renderAdditionalInformation,
            additionalText, currentUserId, sortedParticipantsByScore, attributeToConsider,
            renderPlayerValue, renderPlayerUsernameTableCell, renderInformationOnBottom, idAttributeName,
            columnHeaderText
        } = props;
        ...
        const getPlacementString: any = (player: any, ranked:Array<Array<any>>) => {
            ...
            ranked.forEach((rankedGroup, index) => {
                const isFound = rankedGroup.find(x => x[idAttributeName] === player[idAttributeName])
                if(isFound){
                    placement = index + 1
                    return
                }
        ...
        const getGroupedArrayForRank = (sortedParticipantsByScore: any) => {
            return sortedParticipantsByScore
                .reduce((grouped: Array<Array<GameUser>>, player: any) => {
                    // @ts-ignore
                    const el: Array<GameUser> | undefined = grouped.find((values: Array<GameUser>) => values[0][attributeToConsider] === player[attributeToConsider]
                    );
                    if (el) {
                        el.push(player);
        ...
        const topThree = ranked.slice(0, 3).map((set: any, index: number) =>
            set.map((player: any) => ({
                ...player,
        ...
        return (
            ...
            >
                <Grid xs={12} className={"scoreBoardContainer"}>
                    ...
                    >
                        <Grid xs={12}>
                            {renderAdditionalInformation === undefined ? undefined : renderAdditionalInformation()}
                        </Grid>
                    </Grid>
                                    ...
                                    (
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <p className={`points`}>{trophy[attributeToConsider]} {trophy.additionalText}</p>
                                            </li>
                                        )
                                    }
                                )}
                                                ...
                                                {sortedParticipantsByScore.map((player, index) => (
                                                    ...
                                                    >
                                                        ...
                                                        <TableCell
                                                            ...
                                                            className={"tableColumnEntries"}
                                                        >
                                                            {renderPlayerUsernameTableCell !== undefined ? renderPlayerUsernameTableCell(player) : <>{player.username}</>}
                                                        </TableCell>
                                                        <TableCell
                                                            align="right"
                                                            className={"tableColumnEntries"}
                    ...
                    >
                        <Grid xs>
                            {renderInformationOnBottom === undefined ? undefined : renderInformationOnBottom()}
                        </Grid>
                    </Grid>
                </Grid>
            </Grid>
